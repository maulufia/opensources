...
?>
	<script>alert('로그인 후 이용가능합니다.'); location.href="/auth/login";</script>";
<?php
	}
...
<!--
	...
	<form action="/apply/application" method="POST"><div class="apply-block-wrapper">
		...
		<input class="input-small" type="text" name="represent[]" value="<?= @$applyData[0]['represent[0]'] ?>" />
		<p class="apply-title-detail">소속(팀명/학교 or 회사)</p>
		<p class="apply-title-detail-2">ex) 로켓팀/서울대</p>
		<input class="input-small" type="text" name="represent[]" value="<?= @$applyData[0]['represent[1]'] ?>" />
		<p class="apply-title-detail">휴대폰번호</p>
		<input class="input-small" type="text" name="represent[]" value="<?= @$applyData[0]['represent[2]'] ?>" />
		<p class="apply-title-detail">이메일주소</p>
		<input class="input-small" type="text" name="represent[]" value="<?= @$applyData[0]['represent[3]'] ?>" />
		...
		<input class="input-small" type="text" name="represent[]" value="<?= @$applyData[0]['represent[4]'] ?>" />
		...
		<textarea class="input-big" type="text" name="team[]"><?= @$applyData[0]['team[0]'] ?></textarea>
		<p class="apply-title-detail">팀에서 창출하고자 하는 가치는 무엇인가요?(100자 이내)</p>
		<textarea class="input-big" type="text" name="team[]" onkeyup="countText(this, 100)"><?= @$applyData[0]['team[1]'] ?></textarea>
		...
		<textarea class="input-big-400" type="text" name="team[]" onkeyup="countText(this, 400)" ><?= @$applyData[0]['team[2]'] ?></textarea>
		<p class="count">0/400</p>
		<p class="apply-title-detail">현재 팀이 갖춰야할 역량과 그것을 확보하기 위해 어떤 노력을 하고 있나요?(400자 이내)</p>
		<textarea class="input-big-400" type="text" name="team[]" onkeyup="countText(this, 400)"><?= @$applyData[0]['team[3]'] ?></textarea>
		<p class="count">0/400</p>
		<p class="apply-title-detail">국내외 스타트업 경진대회에서 수상한 경력이 있다면 알려주세요</p>
		<p class="apply-title-detail-2">*일시/대회명/수상기관/상금액/상명</p>
		<textarea class="input-big" type="text" name="team[]"><?= @$applyData[0]['team[4]'] ?></textarea>

		<div class="div-margin"></div>
		<!-- PRODUCT -->
		<p class="apply-title-sub">PRODUCT</p>
		<p class="apply-title-detail">만들고자 하는 프로덕트의 핵심을 요약해서 한 문장으로 작성해 주세요</p>
		<input class="input-small" type="text" name="product[]" value="<?= @$applyData[0]['product[0]'] ?>" />
		<p class="apply-title-detail">프로덕트의 핵심 기능에 기반한 유저 시나리오를 작성해주세요(400자 이내)</p>
		<textarea class="input-big-400" type="text" name="product[]" onkeyup="countText(this, 400)"><?= @$applyData[0]['product[1]'] ?></textarea>
		<p class="count">0/400</p>
		<p class="apply-title-detail">사람들은 왜 이 프로덕트를 사용할까요?(400자 이내)</p>
		<textarea class="input-big-400" type="text" name="product[]" onkeyup="countText(this, 400)"><?= @$applyData[0]['product[2]'] ?></textarea>
		<p class="count">0/400</p>
		...
		<textarea class="input-big-400" type="text" name="product[]" onkeyup="countText(this, 400)"><?= @$applyData[0]['product[3]'] ?></textarea>
		<p class="count">0/400</p>
		<p class="apply-title-detail">이 프로덕트의 사용자가 자신의 친구에게 추천한다면 그 이유는 왜일까요?(100자 이내)</p>
		<textarea class="input-big" type="text" name="product[]" onkeyup="countText(this, 100)"><?= @$applyData[0]['product[4]'] ?></textarea>
		<p class="count">0/100</p>
		<p class="apply-title-detail">이 프로덕트로 어떻게 수익을 창출할 계획인가요?(400자 이내)</p>
		<textarea class="input-big-400" type="text" name="product[]" onkeyup="countText(this, 400)"><?= @$applyData[0]['product[5]'] ?></textarea>
		<p class="count">0/400</p>
		...
		<p class="apply-title-detail-2">ex) youtube, facebook, blog, news, etc...</p>
		<input class="input-small" type="text" name="product[]" value="<?= @$applyData[0]['product[7]'] ?>" />
		...
		<textarea class="input-big" type="text" name="other[]"><?= @$applyData[0]['other[2]'] ?></textarea>
		<p class="apply-title-detail">Be the Rocket에서 지원 받고 싶은 내용을 알려주세요</p>
		<textarea class="input-big" type="text" name="other[]"><?= @$applyData[0]['other[3]'] ?></textarea>
		...
		<div class="radiogroup-wrapper">
			...
			<div class="radiogroup"><input class="" type="radio" name="other[4]" value="4" <?php if(@$applyData[0]['other[4]'] == '4') { echo "checked='checked'"; } ?> /><span class="apply-title-detail-2"> 기타 : </span><input id="others" type="text" name="other[]" value="<?= @$applyData[0]['other[5]'] ?>" /></div>
		</div>
		...
		<textarea class="input-big-400" type="text" name="milestone[]"><?= @$applyData[0]['milestone[0]'] ?></textarea>
		...
		<input class="input-hidden" type="text" name="apply_id" value="<?= $this->session->userdata('id') ?>" />
		<input class="input-hidden" id="submitflag" type="text" name="submitflag" value="tempSave" />
